...
@interface NSString (XHSSZipArchive)
...
- (void)testZipping {
    ...
    BOOL success = [XHSSZipArchive createZipFileAtPath:archivePath withFilesAtPaths:inputPaths];
    ...
    BOOL protected = [XHSSZipArchive isFilePasswordProtectedAtPath:archivePath];
...
- (void)testZippingWithPassword {
    ...
    NSString *archivePath = [outputPath stringByAppendingPathComponent:@"CreatedArchive.zip"];
    BOOL success = [XHSSZipArchive createZipFileAtPath:archivePath withFilesAtPaths:inputPaths withPassword:@"dolphins🐋"];
...
- (void)testZippingWithZeroLengthPassword {
    ...
    NSString *archivePath = [outputPath stringByAppendingPathComponent:@"CreatedArchive.zip"];
    BOOL success = [XHSSZipArchive createZipFileAtPath:archivePath withFilesAtPaths:inputPaths withPassword:@""];
...
- (void)testDirectoryZipping {
    ...
    BOOL success = [XHSSZipArchive createZipFileAtPath:archivePath withContentsOfDirectory:inputPath];
    XCTAssertTrue(success, @"create zip failure");
...
- (void)testUnzipping {
    ...
    BOOL success = [XHSSZipArchive unzipFileAtPath:zipPath toDestination:outputPath delegate:delegate];
...
-(void)testAppendingToZip {
    ...
    XHSSZipArchive* zip = [[XHSSZipArchive alloc] initWithPath:archivePath];
...
- (void)testUnzippingWithPassword {
    ...
    NSError *error = nil;
    id<SSZipArchiveDelegate> delegate = [ProgressDelegate new];
    BOOL success = [XHSSZipArchive unzipFileAtPath:zipPath toDestination:outputPath overwrite:YES password:@"passw0rd" error:&error delegate:delegate];
    XCTAssertTrue(success, @"unzip failure");
...
- (void)testUnzippingWithInvalidPassword {
    ...
    NSError *error = nil;
    id<SSZipArchiveDelegate> delegate = [ProgressDelegate new];
    BOOL success = [XHSSZipArchive unzipFileAtPath:zipPath toDestination:outputPath overwrite:YES password:@"passw0rd123" error:&error delegate:delegate];
...
- (void)testIsPasswordInvalidForArchiveAtPath {
    ...
    BOOL fileHasValidPassword = [XHSSZipArchive isPasswordValidForArchiveAtPath:zipPath password:@"passw0rd" error:&error];
    
    XCTAssertTrue(fileHasValidPassword, @"Valid password reports false.");
    
    BOOL fileHasInvalidValidPassword = [XHSSZipArchive isPasswordValidForArchiveAtPath:zipPath password:@"passw0rd123" error:&error];
...
- (void)testIsFilePasswordProtectedAtPath {
    ...
    BOOL protected = [XHSSZipArchive isFilePasswordProtectedAtPath:zipPath];
    XCTAssertFalse(protected, @"has no password");
    
    
    NSString *zipWithPasswordPath = [[NSBundle bundleForClass:[self class]] pathForResource:@"TestPasswordArchive" ofType:@"zip"];
    
    protected = [XHSSZipArchive isFilePasswordProtectedAtPath:zipWithPasswordPath];
    XCTAssertTrue(protected, @"has password");
}
...
- (void)testZippingAndUnzippingWithUnicodePassword {
    ...
    BOOL success = [XHSSZipArchive createZipFileAtPath:archivePath withFilesAtPaths:inputPaths withPassword:@"ꊐ⌒Ⅳ🤐"];
    ...
    NSError *error = nil;
    id<SSZipArchiveDelegate> delegate = [ProgressDelegate new];
    success = [XHSSZipArchive unzipFileAtPath:archivePath toDestination:outputPath overwrite:YES password:@"ꊐ⌒Ⅳ🤐" error:&error delegate:delegate];
    XCTAssertTrue(success, @"unzip failure");
...
- (void)testZippingWithUnicodeFilenameInside {
    ...
    BOOL success = [XHSSZipArchive createZipFileAtPath:zipPath withFilesAtPaths:@[filePath]];
    XCTAssertTrue(success, @"zip failure");
    
    [[NSFileManager defaultManager] removeItemAtPath:filePath error:nil];
    success = [XHSSZipArchive unzipFileAtPath:zipPath toDestination:workPath delegate:nil];
    XCTAssertTrue(success, @"unzip failure");
...
- (void)testZippingEmptyArchive {
    ...
    BOOL success = [XHSSZipArchive createZipFileAtPath:zipPath withContentsOfDirectory:inputPath keepParentDirectory:NO];
    XCTAssertTrue(success, @"create zip failure");
    success = [XHSSZipArchive createZipFileAtPath:zipPath2 withContentsOfDirectory:inputPath keepParentDirectory:YES];
    XCTAssertTrue(success, @"create zip failure");
...
- (void)testZippingAndUnzippingEmptyDirectoryWithPassword {
    ...
    BOOL success = [XHSSZipArchive createZipFileAtPath:zipPath withContentsOfDirectory:inputPath keepParentDirectory:YES withPassword:@"password"];
    XCTAssertTrue(success, @"create zip failure");
    ...
    id<SSZipArchiveDelegate> delegate = [ProgressDelegate new];
    success = [XHSSZipArchive unzipFileAtPath:zipPath toDestination:outputPath overwrite:YES password:nil error:nil delegate:delegate];
    XCTAssertTrue(success, @"unzip failure");
}
...
- (void)testUnzippingEmptyArchive {
    ...
    BOOL success = [XHSSZipArchive unzipFileAtPath:zipPath toDestination:outputPath delegate:nil];
    XCTAssertTrue(success, @"unzip failure");
}
...
- (void)testZippingAndUnzippingForDate {
    ...
    XCTAssertTrue(success, @"create zip failure");
    id<SSZipArchiveDelegate> delegate = [ProgressDelegate new];
    success = [XHSSZipArchive unzipFileAtPath:archivePath toDestination:outputPath delegate:delegate];
    XCTAssertTrue(success, @"unzip failure");
...
- (void)testZippingAndUnzippingForPermissions {
    ...
    BOOL success = [XHSSZipArchive createZipFileAtPath:archivePath withContentsOfDirectory:inputFile];
    XCTAssertTrue(success, @"create zip failure");

    /********** Un-zipping *******/

    // Using this newly created zip file, unzip it
    success = [XHSSZipArchive unzipFileAtPath:archivePath toDestination:outputDir];
    XCTAssertTrue(success, @"unzip failure");
...
- (void)testUnzippingWithCancel {
    ...
    success = [XHSSZipArchive unzipFileAtPath:zipPath toDestination:outputPath delegate:delegate];
    XCTAssertTrue(success, @"unzip failure");
...
-(void)testShouldProvidePathOfUnzippedFileInDelegateCallback {
    ...
    BOOL success = [XHSSZipArchive unzipFileAtPath:zipPath toDestination:outputPath delegate:collector];
    XCTAssertTrue(success, @"unzip failure");
...
- (void)testPayloadSizeCorrectAndUnpackFileNo {
    ...
    XHSSZipArchive *archive = [[XHSSZipArchive alloc] initWithPath:filePath];
    [archive open];
    ...
    NSNumber *fileSize = [XHSSZipArchive payloadSizeForArchiveAtPath:filePath error:nil];
    ...
    [XHSSZipArchive unzipFileAtPath:filePath toDestination:unpackPath overwrite:true password:password error:nil];
...
- (void)testSymlinkZippingWithFilesAtPaths {
    ...
    success = [XHSSZipArchive createZipFileAtPath:archivePath withFilesAtPaths:files withPassword:nil keepSymlinks:YES];
    XCTAssertTrue(success);
    ...
    //unzip
    success = [XHSSZipArchive unzipFileAtPath:archivePath toDestination:directory];
    XCTAssertTrue(success);
...
- (void)testSymlinkZippingWithContentsOfDirectory {
    ...
    success = [XHSSZipArchive createZipFileAtPath:archivePath
